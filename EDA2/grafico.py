...
while i < tamanhoAVL -1:
    split = yAVL[i].split(":")
    if(split[0] == 'I'):
        vetorInserirAVLx.append(float(split[1]))
        vetorInserirAVLy.append(float(split[2]))
    else:
        vetorRemoverAVLx.append(float(split[1]))
        vetorRemoverAVLy.append(float(split[2]))
    i+=1
...
def grafico_insercao():
    ...
    plt.title("REMOÇÃO", fontsize=20)
    plt.xlabel("Número de elementos", fontsize = 15)
    plt.ylabel("Operações", fontsize = 15)
...
def grafico_remocao():
    ...
    plt.plot(vetorRemoverARNx, vetorRemoverARNy, label = 'Arvore Rubro Negra')
    plt.title("REMOÇÃO")
    plt.yticks(range(0, 45, 5))
